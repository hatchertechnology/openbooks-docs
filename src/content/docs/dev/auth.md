...
admin `user passwd` command makes (see [Overview](/openbooks-docs/dev/api/)).
...
[Overview](/openbooks-docs/dev/api/) for the env var and its default). It
...
- **No `auth login` in any client.** `openbooks-cli`, `openbooks-desktop`,
  ...
  [Desktop overview](/openbooks-docs/use/desktop/), and
  [MCP server](/openbooks-docs/dev/mcp/).
- **No revocation UI.** A token or session is only ended by a password
  change, `user passwd`, or `user rm` — see [Overview](/openbooks-docs/dev/api/).
