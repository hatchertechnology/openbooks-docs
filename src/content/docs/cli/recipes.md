...
Short, task-first examples using the [CLI subcommands](/openbooks-docs/cli/commands/). Assume
...
prefix of a name, or a numeric id — see [command reference](/openbooks-docs/cli/commands/#record)
...
[exit codes](/openbooks-docs/cli/commands/#exit-codes) — so `set -euo pipefail` is enough to
...
from a script, the [TUI's Record tab](/openbooks-docs/cli/tui/#record-tab-only) keeps the date
